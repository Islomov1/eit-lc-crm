...
async function updateReport(formData: FormData) {
  ...
  if (!id || !studentId || !attendance || !homework) return;
...
export default async function StudentPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-10 space-y-8">
      ...
      <div className="bg-white p-6 rounded-2xl shadow">
        <h2 className="font-semibold mb-4">Parents</h2>

        {student.parents.length === 0 && <p>No parents added</p>}

        {student.parents.map((parent) => (
          <div key={parent.id} className="border-b py-2">
            <p className="font-medium">{parent.name}</p>
            <p className="text-sm text-gray-600">Phone: {parent.phone}</p>
            <p className="text-sm text-gray-600">
              Telegram: {parent.telegramId ? parent.telegramId.toString() : "Not set"}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow">
        <h2 className="font-semibold mb-4">Lesson Reports</h2>

        {student.reports.length === 0 && <p>No reports yet</p>}
      ...
      <div className="bg-white p-6 rounded-2xl shadow">
        <h2 className="font-semibold mb-4">Support Sessions</h2>

        {student.supportSessions.length === 0 && <p>No support sessions</p>}
